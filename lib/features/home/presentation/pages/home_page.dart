...
import 'home_page_web.dart'
    if (dart.library.io) 'home_page_stub.dart';
...
class _HomePageState extends ConsumerState<HomePage> {
  ...
  void _showNotificationPermissionBanner() {
    checkAndRequestWebPermission(context);
  }
  ...
  void _setupGlobalCallListener() {
    // ── BUG #13 CORRIGÉ : onIncomingCall est maintenant déclenché dans CallService ──
    ...
    _callService.onCallStatusChanged = (String status) {
      ...
      if (status == 'ended' || status == 'rejected' || status == 'missed') {
        ...
        if (!_isOnCallPage()) NotificationService().cancelAll();
      }
    };
  ...
  bool _isOnCallPage() {
    try {
      return GoRouterState.of(context).matchedLocation.startsWith('/calls/');
    } catch (_) {
      return false;
    }
  }
  ...
  Future<void> _acceptGlobalCall(IncomingCallInfo info) async {
    ...
    if (currentUser == null) {
      AppModal.error(context, title: 'Erreur', message: 'Utilisateur non authentifié.');
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    // Activer le listener de notifications temps réel
    ref.watch(notificationListenerProvider);

    // Abonner WebSocket pour chaque conversation (appels entrants)
...
class _GlobalIncomingCallBanner extends StatefulWidget {
  ...
  @override
  State<_GlobalIncomingCallBanner> createState() => _GlobalIncomingCallBannerState();
}
                        ...
                        child: ListView(
                          ...
                          children: [
                            _SidebarItem(
                              ...
                              label: 'Messages', selected: currentIndex == 0,
                              onTap: () => onNavTap(0),
                            ),
                            _SidebarItem(
                              icon: Icons.call_outlined,
                              selectedIcon: Icons.call_rounded,
                              label: 'Appels', selected: currentIndex == 1,
                              onTap: () => onNavTap(1),
                            ),
                            _SidebarItem(
                              icon: Icons.group_outlined,
                              selectedIcon: Icons.group_rounded,
                              label: 'Groupes', selected: currentIndex == 2,
                              onTap: () => onNavTap(2),
                            ),
                            _SidebarItem(
                              icon: Icons.notifications_outlined,
                              selectedIcon: Icons.notifications_rounded,
                              label: 'Notifications', selected: currentIndex == 3,
                              onTap: () => onNavTap(3),
                            ...
                            if (isAdmin)
                              _SidebarItem(
                                ...
                                label: 'Administration', selected: currentIndex == 4,
                                onTap: () => onNavTap(4),
                              ),
                          ],
                        ),
            ...
            child: Row(
              children: [
                Icon(selected ? selectedIcon : icon,
                  color: selected ? AppColors.primary : AppColors.grey500, size: 20),
                const SizedBox(width: 12),
                Expanded(
                  child: Text(label,
                    style: TextStyle(
      ...
      child: Row(
        ...
        children: [
          ...
          const Text('Version 1.0.0',
            style: TextStyle(fontSize: 12, color: AppColors.grey400, fontFamily: 'Nunito')),
        ],
      ),
    );
  }
